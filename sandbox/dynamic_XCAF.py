...
r"""An example illustrating how to update components in an XDE document
and have the View update interactively.
"""
...
from OCC.Core import XCAFApp, TDocStd, TCollection, XCAFDoc, BRepPrimAPI, \
    Quantity, TopLoc, gp, TPrsStd, XCAFPrs, AIS
...
class MyCanvas(wxViewer3d):
    ...
    def SetBoxPosition(self, val):
        ...
        #  One way to set the location is to remove the label and re-add it.
        # However, this will lose any color or material attributes
        # on the component (although attributes on the underlying Shape
        # are retained). E.g.
        # ~ self.shape_tool.RemoveComponent(self.box_comp2)
        # ~ self.box_comp2 = self.shape_tool.AddComponent(self.top_label,
        #                                                 self.box_label,
        #                                                 loc)
        # I can't see how to make the ShapeTool do this, so we need to edit
        # the TDF_Attribute directly.
    ...
    def ShowAssembly(self, top_label):
        self.aisView = TPrsStd.TPrsStd_AISViewer().New(top_label,
                                                       self.context.GetHandle())
        self.aisPres = TPrsStd.TPrsStd_AISPresentation().Set(top_label,

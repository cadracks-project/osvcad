...
from traitsui.api import View, Item, TreeEditor, TreeNode, ListEditor, VGroup
...
from OCC import XCAFApp, TDocStd, TCollection, TopLoc, TPrsStd, STEPCAFControl, TDF, TDataStd, TNaming, TFunction,\
    TPrsStd
...
tree_ed = TreeEditor(nodes=[TreeNode(node_for=[Label], auto_open=False, children='children', label='repr', view=View()),
                            TreeNode(node_for=[Attribute], auto_open=False, children='', label='cls_name',)],
...
class Doc(HasTraits):
    ...
    traits_view = View(VGroup(Item("filename")), Item("root_label", editor=tree_ed, show_label=False))
...
class App(HasTraits):
    ...
    traits_view = View(Item('docs', style="custom", editor=ListEditor(use_notebook=True), show_label=False), height=600,
